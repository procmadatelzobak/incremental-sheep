...
export const PHASES = {
  1:  { name: 'Stvoření',        gate: s => s.stats.credLifetime >= 2e3,   hint: 'Prodávej vlnu a maso. Vydělej 2 tis. kreditů.' },
  2:  { name: 'Množení',         gate: s => s.stats.credLifetime >= 1e5,   hint: 'Šlechti stádo a prodávej mléko. Vydělej 100 tis. kreditů.' },
  3:  { name: 'Královská',       gate: s => s.stats.credLifetime >= 1e7,   hint: 'Ovládni trh (Monopol). Vydělej 10 mil. kreditů.' },
  4:  { name: 'Nesmrtelnosti',   gate: s => !!s.flags.immortal,            hint: 'Vyrob nápoj nesmrtelnosti (panel Stáda).' },
  5:  { name: 'Moudrých ovcí',   gate: s => s.stats.credLifetime >= 1e10,  hint: 'Šlechti chytré ovce. Vydělej 10 mld. kreditů.' },
  ...
  9:  { name: 'Soudců',          gate: s => s.stats.credLifetime >= 5e11,  hint: 'Spravuj víc stád. Vydělej 500 mld. kreditů.' },
